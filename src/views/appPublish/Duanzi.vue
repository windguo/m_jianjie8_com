  ...
  <div class="container">
    ...
    <form name="add" method="POST" enctype="multipart/form-data" action="http://jianjie.92kaifa.com/e/DoInfo/ecms.php">
        <div class="listTitle">
            ...
            <!--   -->
            <!-- <span v-for="(v,i) in listInfo" :key="i"> -->
              <label>
                <input @click="checkChange(26)" :checked="checkitem(26)" type="radio" name=classid :value="26">段子
              </label>
            <!-- </span> -->
        </div>

        <textarea autofocus="autofocus" placeholder="请输入内容" v-model="message" name="smalltext" cols="60" rows="3" id="smalltext"></textarea>
        <input name="title" type="text" value="message" v-model="message" class="ctitle">
...
export default {
  ...
  mounted() {
    document.title = '发布段子';
    var that = this;
    this.axios.get("/list/new.php?getJson=class").then(res => {
